...
egrep -q '^\s*-a always,exit -F path=/usr/bin/umount -F perm=x -F auid>=1000 -F auid!=4294967295 -k privileged-umount' /etc/audit/rules.d/stig.rules; privileged_umount=$?
if [ $privileged_umount -eq 0 ];
then
