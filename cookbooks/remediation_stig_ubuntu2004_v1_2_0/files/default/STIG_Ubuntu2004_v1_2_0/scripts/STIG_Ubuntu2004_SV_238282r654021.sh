...
egrep -q '^\s*-a always,exit -F path=/sbin/apparmor_parser -F perm=x -F auid>=1000 -F auid!=4294967295 -k perm_chng' /etc/audit/rules.d/stig.rules; apparmor_parser=$?
if [ $apparmor_parser -eq 0 ];
then
