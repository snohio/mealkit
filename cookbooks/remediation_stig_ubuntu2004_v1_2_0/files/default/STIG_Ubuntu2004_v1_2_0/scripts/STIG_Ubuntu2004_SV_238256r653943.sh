...
egrep -q '^\s*-a always,exit -F path=/usr/bin/ssh-agent -F perm=x -F auid>=1000 -F auid!=4294967295 -k privileged-ssh' /etc/audit/rules.d/stig.rules; privileged_ssh=$?
if [ $privileged_ssh -eq 0 ];
then
