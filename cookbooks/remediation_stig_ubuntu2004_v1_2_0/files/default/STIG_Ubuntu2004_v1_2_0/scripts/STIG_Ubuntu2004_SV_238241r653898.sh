...
sudo egrep "^-w\s+/etc/gshadow \s+-p\s+wa\s+-k\s+usergroup_modification\s*$" /etc/audit/rules.d/stig.rules || sudo echo "-w /etc/gshadow  -p wa -k usergroup_modification" >> /etc/audit/rules.d/stig.rules
