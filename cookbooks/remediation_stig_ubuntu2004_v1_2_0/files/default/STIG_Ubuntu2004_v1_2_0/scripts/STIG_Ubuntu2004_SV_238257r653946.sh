...
egrep -q '^\s*-a always,exit -F path=/usr/lib/openssh/ssh-keysign -F perm=x -F auid>=1000 -F auid!=4294967295 -k privileged-ssh' /etc/audit/rules.d/stig.rules; ssh_keysign=$?
if [ $ssh_keysign -eq 0 ];
then
