...
egrep -q '^\s*-a always,exit -F path=/usr/bin/mount -F perm=x -F auid>=1000 -F auid!=4294967295 -k privileged-mount' /etc/audit/rules.d/stig.rules; privileged_mount=$?
if [ $privileged_mount -eq 0 ];
then
